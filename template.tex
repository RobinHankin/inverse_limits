...
for example, $(f\cup g)(x) := f(x)\cup g(x)$.  We define a {\em
  partial} function to be a (set-valued) function whose range includes
$\emptyset$.  Conversely, if the image of every $x\in X$ is nonempty,
$\emptyset\not\in\operatorname{Range}(f)$, we say that $f$ is a {\em
  total} function.  We define the {\em constant function} $f_U(\cdot)$
to be the function with $f_U(x)=\lbrace U\rbrace\,\forall x\in X$,
for some $U\subseteq X$.  Given a set-valued function $f\colon X
...
It is intuitive that when we take the union of two functions, the
inverse limit of the resulting function is a superset of the union of
the inverse limits of the two functions.

 \begin{thm}
The inverse limit of $f\cup g$ is a superset of the union of the inverse limits of $f$ and $g$ separately.
 \end{thm}
 \begin{proof}
 We are trying to show that $\invlim{f} \cup \invlim{g} \subseteq\invlim{f\cup g}$.
 Formally, if $S = (x_0 , x_1 , x_2 , \ldots)\in\invlim{f}\cup\invlim{g}$ then
 either $S\in\invlim{f}$ or $S\in\invlim{g}$. 
 Thus, for $i = 0, 1, 2,\ldots$,  $x_i\in f(x_{i+1})\cup g(x_{i+1})$.
 So $x_i \in ( f  \cup  g )(x_{i+1})$ and so $S\in\invlim{f\cup g}$.
 \end{proof}
...
a\right\rbrace\,\forall x\in X$ and, with some abuse of notation,
...
\begin{exam}
  In figure~\ref{strictly}, we have $f(x)=\lbrace a\rbrace$ and
  $g(x)=\lbrace b\rbrace$ (with $a\neq b$) for $x\in[0,1]$.  Now
  $\invlim{f}=a^\infty$ and $\invlim{b}=b^\infty$, but $\invlim{f \cup
    g}$ is precisely sequences of the form $(x_0,x_1,x_2,\ldots)$
  where $x_i\in\lbrace a,b\rbrace$ for $i\geqslant 0$.  In particular,
  this point contains $(a,b,\ldots)$ which occurs in neither of the
  separate inverse limits.
\end{exam}
...
These examples would suggest the natural conjecture that the inverse
...
Recall that the constant function $f_{\lbrace U\rbrace}(\cdot)$ is
defined by $f_U(x)=\lbrace U\rbrace\,\forall x\in X$.  Here I consider
unions of $f_{\lbrace a\rbrace}$ for some $a\in X$ with a partial
function $g$ that consists of just one point:
...
\begin{itemize}
  \item General position: $a\neq b$, $b\neq c$, $c\neq a$.  Then $\invlim{f\cup g}=a^\infty$.
  \item $a\neq b=c$.  Then $\invlim{f\cup g}=\bigcup_{i\geqslant 0}b^na^\infty$, a convergent sequence.
  \item $a=b\neq c$ (figure~\ref{figs567}).  Then
    \begin{multline}
      \invlim{f\cup g}=
      a^\infty\cup
...
The third case is by far the most interesting.  We note in passing
that the inverse limit is not identical to the underlying set of the
finitely presented group $\left\langle
...
Consider the function $\fa$ for some $a\in X$.
Suppose $g$ is a set valued function such that $\fa\cap g=\emptyset$ and
$\invlim{\fa}\cup\invlim{g} = \invlim{\fa\cup g}$.  Then $G(g)$ must not
...
for some $x_0\neq a$.  Then $S=(ax_0)^\infty\in\invlim{f\cup g}$.
This is not in the inverse limit of $f$ since $\invlim{f} = a^\infty$.
It is also not in the inverse limit of $g$, for the point for $(x_0,a)
\in G(f)$ and $f\cap g=\emptyset$.  So $S\notin \invlim{g}$ and thus
$\invlim{f}\cup\invlim{g}$ is a strict subset of $\invlim{f\cup g}$.

Now suppose that $G(g)$ intersects the (diagonal) line $y = x$ at the
point $(x_0,x_0)$ for some $x_0\neq a$; that is, $x_0\in f(x_0)$.
...
However, the converse of theorem~\ref{fish} is false.  For a
counterexample, see figure~\ref{notyetdone}, where we take $f$ to be
$f_{\lbrace 1\rbrace}$ and
...
for some $a\in(0,1)$.  We see that $f\cap g_{\lbrace
  1,2\rbrace}=\emptyset$ and $\invlim{f}=1^\infty$,
...
\caption[width=\textwidth]{Diagramatic illustration of the
  \label{semicircle} inverse limit of
  a function}
\end{figure}
